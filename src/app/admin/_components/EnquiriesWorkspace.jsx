...
import { AlertTriangle, ArrowRight, BarChart3, CalendarClock, ChevronLeft, ChevronRight, CircleCheck, Download, Filter, Inbox, LoaderCircle, Mail, MessageCircle, Plus, RefreshCw, Search, Target, UserRoundPlus, UsersRound, X } from "lucide-react";
...
export function EnquiriesWorkspace({ initialResult, initialFilters = {}, assignees = [], title = "Enquiries & leads", description = "Manage every website enquiry from first contact to investor handoff.", showAnalytics = true, canManage = false, canAssign = false, canAnalytics = false }) {
  ...
  async function load(next = {}, { resetCursor = false } = {}) {
  ...
  async function goNext() {
    ...
    await load({ cursor: result.nextCursor, page: (filters.page || 1) + 1 });
  }
  ...
  async function goPrevious() {
    ...
    await load({ cursor: previous, page: Math.max(1, (filters.page || 1) - 1) });
  }

  async function rebuildDirectory() {
    setBusy(true);
    try {
      const response = await fetch("/api/admin/enquiries/reindex", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ limitPerSource: 10000 }) });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Unable to rebuild the enquiry directory.");
  ...
  return <div><div className="mb-7 flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#1F4ED8]">Client acquisition</p><h1 className="mt-2 font-serif text-3xl font-bold sm:text-4xl">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#6B7280]">{description}</p></div><div className="flex flex-wrap gap-2">{canAnalytics?<Link href="/admin/enquiries/analytics" className="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm font-bold text-[#1F4ED8]"><BarChart3 size={16}/>Analytics</Link>:null}{canAnalytics?<a href="/api/admin/enquiries/export" className="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm font-bold text-[#1F4ED8]"><Download size={16}/>Export CSV</a>:null}{canManage?<button type="button" onClick={rebuildDirectory} disabled={busy} className="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm font-bold text-[#1F4ED8] disabled:opacity-50"><RefreshCw size={16}/>Rebuild index</button>:null}{canManage?<button type="button" onClick={()=>setManualOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#1F4ED8] px-5 py-3 text-sm font-bold text-white"><Plus size={17}/>Add enquiry</button>:null}</div></div>
    ...
    <div className="mt-7 overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm"><div className="border-b border-black/5 p-4 sm:p-5"><form onSubmit={(event)=>{event.preventDefault();load({}, { resetCursor: true });}} className="grid gap-3 lg:grid-cols-[minmax(220px,1fr)_repeat(5,minmax(130px,auto))_auto]"><label className="relative"><Search size={16} className="pointer-events-none absolute left-3 top-3.5 text-[#6B7280]"/><input value={filters.search} onChange={(event)=>setFilters({...filters,search:event.target.value})} placeholder="Search name, email, phone or reference" className="h-11 w-full rounded-xl border border-gray-200 pl-10 pr-3 text-sm"/></label><select value={filters.source} onChange={(event)=>load({source:event.target.value}, { resetCursor: true })} className="h-11 rounded-xl border border-gray-200 bg-white px-3 text-sm">{SOURCE_OPTIONS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select value={filters.status} onChange={(event)=>load({status:event.target.value}, { resetCursor: true })} className="h-11 rounded-xl border border-gray-200 bg-white px-3 text-sm">{STATUS_OPTIONS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select value={filters.priority} onChange={(event)=>load({priority:event.target.value}, { resetCursor: true })} className="h-11 rounded-xl border border-gray-200 bg-white px-3 text-sm">{PRIORITY_OPTIONS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select value={filters.assignee} onChange={(event)=>load({assignee:event.target.value}, { resetCursor: true })} className="h-11 rounded-xl border border-gray-200 bg-white px-3 text-sm"><option value="all">All assignees</option><option value="unassigned">Unassigned</option>{assignees.map((item)=><option key={item.uid} value={item.uid}>{item.displayName}</option>)}</select><select value={filters.followUp} onChange={(event)=>load({followUp:event.target.value}, { resetCursor: true })} className="h-11 rounded-xl border border-gray-200 bg-white px-3 text-sm"><option value="all">All follow-ups</option><option value="due">Due now</option><option value="scheduled">Scheduled</option></select><button disabled={busy} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-gray-200 px-4 text-sm font-bold text-[#1F4ED8]">{busy?<LoaderCircle size={16} className="animate-spin"/>:<Filter size={16}/>}Apply</button></form></div>
      ...
      <div className="flex flex-col gap-3 border-t border-black/5 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-[#6B7280]">Showing {result.items.length} of {result.total} matching enquiries</p><div className="flex items-center gap-2"><button type="button" disabled={busy||!cursorStack.length} onClick={goPrevious} className="rounded-lg border border-gray-200 p-2 disabled:opacity-40"><ChevronLeft size={16}/></button><span className="px-2 text-xs font-semibold">Page {filters.page || 1}</span><button type="button" disabled={busy||!result.hasMore||!result.nextCursor} onClick={goNext} className="rounded-lg border border-gray-200 p-2 disabled:opacity-40"><ChevronRight size={16}/></button></div></div></div>
