...
export function WebsitePageEditor({ pageKey, initialItem }) {
  ...
  return (
    <form onSubmit={save} className="space-y-6">
      ...
      <section className="rounded-2xl border border-black/5 bg-white p-5 shadow-sm sm:p-6"><h2 className="font-serif text-2xl font-bold">Page and SEO</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Admin page title" value={form.title} onChange={(value) => setRoot("title", value)} required /><Field label="SEO title" value={form.seo?.title} onChange={(value) => setSeo("title", value)} /><div className="sm:col-span-2"><Field label="Meta description" value={form.seo?.description} onChange={(value) => setSeo("description", value)} textarea rows={3} /></div><label className="flex items-center gap-3 text-sm font-semibold"><input type="checkbox" checked={form.seo?.allowIndexing !== false} onChange={(event) => setSeo("allowIndexing", event.target.checked)} className="h-4 w-4 accent-[#1F4ED8]" />Allow public indexing when published</label></div></section>
