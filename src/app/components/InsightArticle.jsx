...
function Block({ block }) {
  ...
  if (block.type === "image" && block.url) return <figure className="my-10"><div className="relative aspect-[16/9] overflow-hidden rounded-2xl bg-[#F4F6F9]"><OptimizedImage src={block.url} alt={block.altText || ""} fill sizes="(max-width: 767px) 100vw, 760px" className="h-full w-full object-cover" style={{ objectPosition: `${block.focalX ?? 50}% ${block.focalY ?? 50}%` }} /></div>{block.caption && <figcaption className="mt-3 text-center text-xs leading-5 text-[#6B7280]">{block.caption}</figcaption>}</figure>;
...
export function InsightArticle({ post, author, categories, relatedPosts = [], previewMode = false }) {
  ...
  return <article>
    ...
    {post.featuredImage?.url && <div className="bg-white"><figure className="mx-auto max-w-[1080px] px-5 pt-12 sm:px-6 lg:px-8"><div className="relative aspect-[16/9] overflow-hidden rounded-3xl bg-[#F4F6F9]"><OptimizedImage src={post.featuredImage.url} alt={post.featuredImage.altText || ""} fill priority sizes="(max-width: 767px) 100vw, 1080px" className="h-full w-full object-cover" style={{ objectPosition: `${post.featuredImage.focalX ?? 50}% ${post.featuredImage.focalY ?? 50}%` }} /></div>{post.featuredImage.caption && <figcaption className="mt-3 text-center text-xs text-[#6B7280]">{post.featuredImage.caption}</figcaption>}</figure></div>}
