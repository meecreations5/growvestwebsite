...
function validateForm(body) {
  const form = {
    ...
    clarity: cleanText(body.clarity, 160),
  ...
  if (!form.name || !form.email || !form.phone || !form.clarity || !form.consent) {
  ...
  if (!CLARITY_OPTIONS.has(form.clarity)) {
    throw new ApiError(400, "Please select what you would like greater clarity around.", "INVALID_CLARITY_AREA");
  }
...
export async function POST(request) {
  ...
  try {
    ...
    requestId = `GV-IX-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
    ...
    await leadReference.set({
      ...
      sourcePage: form.sourcePage || "/investor-experience",
      campaign: form.campaign,
      tags: ["investor-experience", "campaign"],
      status: "new",
      priority: "normal",
      assignedTo: null,
      assignedToName: "",
    ...
    await createLeadActivityForPublicSubmission({
      leadKey,
      summary: "Received a new Investor Experience campaign enquiry.",
      details: {
        sourcePage: form.sourcePage || "/investor-experience",
        clarityArea: form.clarity,
        campaign: form.campaign,
      },
    }).catch(() => {});
    ...
    const detailsHtml = `
      <h2>New GrowVest Investor Experience campaign enquiry</h2>
      ...
      <p><strong>Greater clarity around:</strong> ${escapeHtml(form.clarity)}</p>
      <p><strong>Source page:</strong> ${escapeHtml(form.sourcePage || "/investor-experience")}</p>
      <p><strong>Campaign source:</strong> ${escapeHtml(form.campaign.source || "Not provided")}</p>
      <p><strong>Campaign medium:</strong> ${escapeHtml(form.campaign.medium || "Not provided")}</p>
      <p><strong>Campaign name:</strong> ${escapeHtml(form.campaign.campaign || "Not provided")}</p>
      <p><strong>Campaign content:</strong> ${escapeHtml(form.campaign.content || "Not provided")}</p>
      <p><strong>Campaign term:</strong> ${escapeHtml(form.campaign.term || "Not provided")}</p>
    ...
    const teamResult = await sendTransactionalEmail({
      ...
      subject: `Investor Experience enquiry ${requestId}: ${form.name}`,
      ...
      await writeCommunicationLog(db, {
        ...
        entityId: requestId,
        type: "investor_experience_team_notification",
    ...
    const visitorResult = await sendTransactionalEmail({
      ...
      subject: `GrowVest has received your request - ${requestId}`,
      htmlContent: `
        <p>Dear ${escapeHtml(form.name)},</p>
        <p>Thank you for reaching out.</p>
        <p>A member of the GrowVest team will connect with you about the area where you would like greater clarity.</p>
        <p><strong>Reference:</strong> ${escapeHtml(requestId)}</p>
      ...
      await writeCommunicationLog(db, {
        ...
        entityType: "websiteLead",
        entityId: requestId,
        type: "investor_experience_visitor_acknowledgement",
